...
export default function AIAgentsPage() {
  ...
  const techStack = [
    {
      title: t('techStack.llms'),
      items: t.raw('techStack.llmsItems') as string[]
    },
    {
    ...
    {
      title: t('techStack.channels'),
      items: t.raw('techStack.channelsItems') as string[]
    },
    {
      ...
      <section className="relative py-32 px-6 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          >
            ...
            <h1 className="text-5xl md:text-6xl lg:text-7xl font-bold mb-6 bg-gradient-to-r from-blue-400 via-cyan-400 to-purple-400 bg-clip-text text-transparent">
              {t('hero.subtitle')}
            </h1>
          ...
          <div className="space-y-6">
            {process.map((step, index) => (
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ delay: index * 0.1 }}
                className="flex gap-6 items-start bg-slate-800/30 border border-slate-700 rounded-xl p-6 hover:border-cyan-500/50 transition-all duration-300"
              >
                <div className="flex-shrink-0 w-12 h-12 rounded-full bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center font-bold text-xl">
                  {index + 1}
                </div>
                <div>
      ...
      <section className="py-20 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            {/* Project 4 - AI WhatsApp Packages */}
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ delay: 0.3 }}
              className="bg-gradient-to-br from-orange-600/20 to-red-600/20 border border-orange-500/30 rounded-2xl p-8"
            >
              <div className="flex items-start gap-4 mb-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                {(t.raw('realProjects.project4.metrics') as string[]).map(
                  (metric, index) => (
                    <div key={index} className="flex items-start gap-2">
                      <CheckCircle2 className="w-5 h-5 text-green-400 flex-shrink-0 mt-0.5" />
                      <p className="text-sm text-orange-400">{metric}</p>
                    </div>
                  )
                )}
              </div>
      ...
      <section className="py-20 px-6 bg-slate-900/50">
        <div className="max-w-4xl mx-auto">
          <motion.div
            ...
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="bg-gradient-to-br from-blue-600/20 to-cyan-600/20 border border-blue-500/30 rounded-2xl p-12 text-center"
          >
